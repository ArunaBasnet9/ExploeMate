      ...
      <div className="dash-nav-mobile md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-xl border border-white/50 px-8 py-4 rounded-full shadow-2xl z-40 flex items-center gap-8 ring-1 ring-slate-100">
        {navItems.map((item, i) => (
          <button 
            key={item.label} 
            onClick={() => onNavigate(item.page)}
            className={`flex flex-col items-center gap-1 transition-all duration-300 relative group ${item.page === 'saved' ? 'text-sky-600 scale-110' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <div className={`transition-transform duration-300 ${item.page === 'saved' ? '-translate-y-1' : 'group-hover:-translate-y-2'}`}>
                <item.icon size={24} className={item.page === 'saved' ? 'fill-sky-100' : ''} />
            </div>
            {item.page === 'saved' && <div className="absolute -bottom-2 w-1.5 h-1.5 bg-sky-600 rounded-full shadow-lg shadow-sky-400/50"></div>}
          </button>
        ))}
      </div>
    </div>
