...
export const Footer = ({ onNavigate }: { onNavigate: (page: string) => void }) => (
  <footer className="bg-white pt-20 md:pt-24 pb-10 md:pb-12 border-t border-slate-100 relative z-10">
...
export const Navbar = ({ onNavigate, isLoggedIn = false }: { onNavigate: (page: string) => void, isLoggedIn?: boolean }) => {
  ...
  const textColor = scrolled ? 'text-sky-50 hover:text-white' : 'text-slate-600 hover:text-sky-600';
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 px-4 md:px-0 ${scrolled ? 'bg-sky-600/90 backdrop-blur-xl shadow-lg py-2.5 md:py-3' : 'bg-transparent py-5 md:py-6'}`}>
      <div className="max-w-7xl mx-auto md:px-8 flex items-center justify-between">
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onNavigate('landing')}>
          <div className={`${logoBg} p-1.5 rounded-xl shadow-sm transition-colors duration-300 border border-white/10`}>
            <Logo className="w-7 h-7 md:w-8 md:h-8" variant={scrolled ? "white" : "default"} />
          </div>
          <span className={`font-display font-bold text-lg md:text-xl tracking-tight transition-colors duration-300 ${logoText}`}>ExploreMate</span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          {isLoggedIn ? (
             <button onClick={() => onNavigate('dashboard')} className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all shadow-lg shadow-sky-900/10 ${scrolled ? 'bg-white text-sky-700 hover:bg-sky-50' : 'bg-sky-900 text-white hover:bg-sky-800'}`}>
               Dashboard
             </button>
          ) : (
            <>
              <button onClick={() => onNavigate('login')} className={`text-sm font-bold transition-colors ${scrolled ? 'text-white hover:text-sky-100' : 'text-slate-700 hover:text-sky-600'}`}>
                Log In
              </button>
              <button onClick={() => onNavigate('signup')} className={`px-5 lg:px-6 py-2.5 rounded-full text-sm font-bold transition-all shadow-lg shadow-sky-900/10 ${scrolled ? 'bg-white text-sky-700 hover:bg-sky-50' : 'bg-sky-900 text-white hover:bg-sky-800'}`}>
                Sign Up
              </button>
            </>
          )}
        ...
        {/* Mobile Toggle */}
        <button 
            className={`md:hidden p-2 rounded-xl transition-colors ${scrolled ? 'text-white bg-white/10' : 'text-slate-700 bg-slate-100'}`} 
